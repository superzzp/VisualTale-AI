...
import { inputDataModel, inputImageModel, promptPresets, segmentRequestPrompt } from "../utils/SelectorOptions"
...
function FunWithAI() {
  ...
  const [selectedDataModel, setDataModel] = useState('text-davinci-003');
  const [selectedImageModel, setImageModel] = useState('dall-e-001');
  ...
  // Helper functions
  function addResultToUI(currID, inputText, fullTextResponse, textSegmentsWithImageURLs) {
    setResultsList(resultsList.concat([{ id: currID, prompt: inputText, fullTextResponse: fullTextResponse, textSegmentsWithImageURLs: textSegmentsWithImageURLs}]));
  }
  ...
  async function onTextFormSubmit(event) {
    ...
    try {
      ...
      if (selectedImageModel == 'dall-e-001') {
        fullPrompt += segmentRequestPrompt
      }
      const response = await OpenAIServices.textCompletion(fullPrompt, selectedMaxResTokenLength, selectedTemperature, selectedTopP, selectedFreqPenalty, selectedPresPenalty, selectedDataModel)
      const responseStatus = response.status
      const responseJSON = await response.json()
      const responseText = responseJSON.text.trim()
      const currID = resultsList.length
      let textSegmentsWithImageURLs = null
      if (responseStatus == 200) {
        if (selectedImageModel == 'dall-e-001') {
          // regex for searching the response text, for finding the story of each segment, and their associated image description, 
          // generated by openai
          const regex1 = /Segment \d+:/g
          var match = ""
          const segmentStartIdx = []
          const imgDescStartIdx = []
          const parsedRespTexts = []
          
          while ((match = regex1.exec(responseText)) != null) {
            //console.log("match found at " + match.index);
            segmentStartIdx.push(match.index)
          }
          const regex2 = /Image Description:/g
          while ((match = regex2.exec(responseText)) != null) {
            //console.log("Img des match found at " + match.index);
            imgDescStartIdx.push(match.index)
          }
          if (segmentStartIdx.length == imgDescStartIdx.length &&
            segmentStartIdx.length != 0 && imgDescStartIdx != 0) {
            for (let i = 0; i < segmentStartIdx.length; i++) {
              const curSegIdx = segmentStartIdx[i]
              const currImgIdx = imgDescStartIdx[i]
              const currSegText = (i == 0 && segmentStartIdx[i] != 0) ? 
                responseText.substring(11, currImgIdx).trim() : responseText.substring(curSegIdx + 11, currImgIdx).trim()
              const currImgText = (i == segmentStartIdx.length-1) ? 
                responseText.substring(currImgIdx + 18, responseText.length).trim() : responseText.substring(currImgIdx + 18, segmentStartIdx[i+1]).trim()
                //console.log(currSegText, currImgText)
              parsedRespTexts.push([currSegText, currImgText])
            }
            const promises = []
            for (let t of parsedRespTexts) {
              //console.log(t)
              promises.push(OpenAIServices.imageGeneration(t[1]))
            }
            const responses = await Promise.all(promises)
            textSegmentsWithImageURLs = []
            for (let i in responses) {
              const result = await responses[i].json()
              //console.log(result)
              textSegmentsWithImageURLs.push({
                original: result.imgURL,
                thumbnail: result.imgURL,
                description: parsedRespTexts[i][0],
              })
            }
          } else {
            console.log('Issue segmenting text')
            console.log(segmentStartIdx)
            console.log(imgDescStartIdx)
          }
          addResultToUI(currID, inputText, responseText, textSegmentsWithImageURLs);          
        } else {
          addResultToUI(currID, inputText, responseText, textSegmentsWithImageURLs);
        }
        LocalStorage.saveResultToLocalStorage(currID, inputText, responseText, textSegmentsWithImageURLs);
      } else {
        console.log('server side error submitting text')
      }
      setLoading(false);
    } catch (err){
      console.log(err)
              ...
              {displayResponses ?
                <div id="responses">
                  ...
                  <div className="container" id="results">
                    {resultsList.length > 0 && resultsList[resultsList.length-1].textSegmentsWithImageURLs != null?
                      <MyGallery images={resultsList[resultsList.length-1].textSegmentsWithImageURLs}></MyGallery>:null
                    }
                    {resultsList.slice(0).reverse().map((item) => {
                      return (
                        <TwoColsGrid key={item.id} prompt={item.prompt} response={item.fullTextResponse}></TwoColsGrid>
                      )
                    })}
              ...
              <div className='pg-right-content'>
                <div className='parameter-panel'>
                  ...
                  <div className='parameter-panel-grid'>
                    ...
                    <div>
                      <div className={["body-small", "control-label"].join(" ")}>Text Completion Model</div>
                      <Selector onSelectorChange={updateDataModel} options={inputDataModel} width={216} default={inputDataModel[0]}></Selector>
                    </div>
                    {/* OpenAI Image Model */}
                    <div>
                      <div className={["body-small", "control-label"].join(" ")}>Image Generation Model</div>
                      <Selector onSelectorChange={updateImageModel} options={inputImageModel} width={216} default={inputImageModel[0]}></Selector>
                    </div>
                    {/* Temperature */}
                    <div>
